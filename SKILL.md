...
- Use `scripts/transcription_postprocess.py` for deterministic Bilibili URL selection, subtitle URL extraction, audio verification, metadata building, transcript inspection, slice merging, deliverable generation, and package verification instead of retyping ad hoc Python snippets.
...
- Save deliverables under a concrete directory such as `~/Downloads/<source>_transcripts/<episode_id>/` and return exact paths plus Feishu `MEDIA:` attachments when available.
...
1. **Find the audio source**
   ...
   - For Bilibili pages, also try to find `x/v2/subtitle/web/view` in page resources. Download the response, then run `scripts/transcription_postprocess.py extract-subtitle-url subtitle_view.bin --out subtitle_body_url.txt --report subtitle_extract.json --allow-missing`. If a usable Bilibili subtitle body can be downloaded and it covers the full video, prefer it as the primary transcript source unless the user explicitly asks for Whisper/local transcription; otherwise use it to correct Whisper names/tickers/uncertain regions. If it cannot be decoded, downloaded, or coverage is incomplete, record the failure and continue with Whisper.
